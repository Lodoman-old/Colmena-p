...
class EventService {
    ...
    async crearEventoConGeocerca(input) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`INSERT INTO eventos (nombre, descripcion, fecha_inicio, fecha_fin,
    ...
    async ciudadanosEnRadio(eventoId) {
      ...
      WHERE e.id = $1
      ORDER BY distancia_metros ASC`, [eventoId]);
    ...
    async alertasPendientesEnvio(limite = 50) {
        const result = await this.pool.query(`SELECT a.*, c.nombre as ciudadano_nombre, c.telefono,
    ...
    async marcarAlertaEnviada(alertaId) {
        await this.pool.query(`UPDATE alertas_whatsapp
       SET enviado = TRUE, timestamp_envio = NOW()
       WHERE id = $1`, [alertaId]);
    }
    async incrementarReintento(alertaId) {
        await this.pool.query(`UPDATE alertas_whatsapp
       SET retry_count = retry_count + 1,
           error_envio = 'Reintento automático'
       WHERE id = $1`, [alertaId]);
    }
    async obtenerGeocercasActivas(sectorId) {
        let query = `
    ...
    async desactivarGeocerca(geocercaId) {
        await this.pool.query(`UPDATE geofences SET activo = FALSE WHERE id = $1`, [geocercaId]);
    }
    async proximidadCiudadano(ciudadanoId, geocercaId) {
        const result = await this.pool.query(`SELECT ST_Distance(c.ubicacion::geometry, g.ubicacion::geometry) as distancia
       FROM ciudadanos c, geofences g
       WHERE c.id = $1 AND g.id = $2`, [ciudadanoId, geocercaId]);
