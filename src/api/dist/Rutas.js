...
class RoutingService {
    ...
    construirWhereFiltros(filtros, params) {
        const conds = [];
        ...
        if (Array.isArray(filtros.discapacidad_ids) && filtros.discapacidad_ids.length) {
            params.push(filtros.discapacidad_ids.map((x) => parseInt(x)).filter((n) => !Number.isNaN(n)));
            conds.push(`c.discapacidad_id = ANY($${params.length})`);
        }
        if (Array.isArray(filtros.ocupacion_ids) && filtros.ocupacion_ids.length) {
            params.push(filtros.ocupacion_ids.map((x) => parseInt(x)).filter((n) => !Number.isNaN(n)));
    ...
    async calcularRutaOptima(origen, seccionId, tipo = 'encuesta', maxDistanciaKm = 25) {
        ...
        const paradasFiltradas = paradasConDistancia.filter(p => p.distancia_desde_origen <= maxDistanciaKm);
        if (paradasFiltradas.length !== paradasConDistancia.length) {
            advertencias.push(`${paradasConDistancia.length - paradasFiltradas.length} paradas exceden la distancia máxima de ${maxDistanciaKm}km`);
        }
    ...
    async obtenerParadas(seccionId, tipo, filtros) {
        let query;
        ...
        try {
            ...
            if (response.data?.distances?.[0]) {
                return paradas.map((p, i) => ({
                    ...p,
                    distancia_desde_origen: Math.round((response.data.distances[0][i + 1] || 0) / 1000)
                }));
            }
        }
        catch {
    ...
    async repartirRutas(seccionId, tipo, numGrupos, filtros) {
    ...
    async optimizarConOSRM(origen, paradas) {
        if (paradas.length === 0) {
        ...
        try {
            ...
            try {
                ...
                const routeRes = await axios_1.default.get(routeUrl, {
                    params: {
                        overview: 'full',
                        geometries: 'geojson',
                        steps: 'false'
            ...
            return {
                waypoints: paradas,
                distancia_total_km: paradas.reduce((sum, p, i) => sum + this.distanciaHaversine(i === 0 ? origen : paradas[i - 1].ubicacion, p.ubicacion), 0),
                tiempo_total_minutos: Math.round(paradas.length * 10),
                geometry: []
            };
        }
    }
    async calcularOrdenTSP(paradas) {
        ...
        let matriz = [];
        try {
            const response = await axios_1.default.get(`${OSRM_BASE_URL}/table/v1/foot/${coords}`, {
        ...
        const totalDistancia = (orden) => {
            let s = 0;
            for (let i = 0; i < orden.length - 1; i++)
                s += dist(orden[i], orden[i + 1]);
            return s;
        };
    ...
    async obtenerCentroideSeccion(seccionId) {
        try {
      ...
      `, [seccionId]);
            if (res.rows[0]?.lat && res.rows[0]?.lng)
                return { lat: parseFloat(res.rows[0].lat), lng: parseFloat(res.rows[0].lng) };
        }
        catch { }
        return null;
    }
    distanciaHaversine(a, b) {
        const R = 6371;
